...
func TestFiltersAggregationFilters(t *testing.T) {
	f1 := elastic.NewRangeQuery("stock").Gt(0)
	f2 := elastic.NewTermQuery("symbol", "GOOG")
...
func TestFiltersAggregationWithKeyedAndNonKeyedFilters(t *testing.T) {
	agg := NewFiltersAggregation().
		Filter(elastic.NewTermQuery("symbol", "MSFT")).			// unnamed
		FilterWithName("one", elastic.NewTermQuery("symbol", "GOOG"))	// named filter
	_, err := agg.Source()
	if err == nil {
